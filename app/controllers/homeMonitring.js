	...
	db.user.authUser(token).then(function(response) {
			if (!!id && !!token) {
				if (response != '' && response != null) {
					...
					var sql = "UPDATE userProfessionalRegistration SET status='1' WHERE id='" + id + "'";
...
function email_notification(to, body) {
	...
	db.user.checkByuserid(to).then(function(response) {
		...
		smtpTransport.sendMail({ //email options
			...
			html: body + "<br><br>Best Regards <br>CHIEF Support<br><br><hr><br>This message was sent to " + email + ". If you don't want to receive these emails from CHIEF in the future, please update your notification settings." // body
	...
	db.user.authUser(token).then(function(response) {
			...
			if (!!userid && !!token) {
				if (response != '' && response != null) {
					...
					db.userProfessionalRegistration.gethypertensionpatientsNotControlledDetailSystolic(email).then(function(response) {
	...
	db.user.authUser(token).then(function(response) {
			...
			if (!!userid && !!token) {
				if (response != '' && response != null) {
					...
					db.userProfessionalRegistration.gethypertensionpatientsDetailDiastolic(email).then(function(response) {
	...
	db.user.authUser(token).then(function(response) {
			...
			if (!!email && !!token) {
				if (response != '' && response != null) {
					...
					db.userProfessionalRegistration.gethypertensionpatientsWellControlledMorningDetails(email).then(function(response) {
	...
	db.user.authUser(token).then(function(response) {
			...
			if (!!email && !!token) {
				if (response != '' && response != null) {
					...
					db.userProfessionalRegistration.gethypertensionpatientsWellControlledEveningDetails(email).then(function(response) {
			...
			if (!!id && !!token) {
				if (response != '' && response != null) {
					...
					if (!!bp_id) { ///update bp vlaues
						...
						var updatesqlillness = "UPDATE  bp_goal SET systolic_to='" + SBP + "', diastolic_to='" + DBP + "' WHERE id='" + bp_id + "'";
					 ...
					 **for blood glucose
					 ***************/
					if (!!glucose_id) { ///update bp vlaues

						var updatesqlillness = "UPDATE  glucose_goal SET goalBloodGlucose_Start_BM='" + diabetes_pre_control_from + "', goalBloodGlucose_End_BM='" + diabetes_pre_control_to + "',goalBloodGlucose_Start_AM='" + diabetes_post_control_from + "', goalBloodGlucose_End_AM='" + diabetes_post_control_to + "' WHERE id='" + glucose_id + "'";
	...
	db.user.authUser(token).then(function(response) {
			...
			if (!!token) {
				if (response != '' && response != null) {
					...
					db.userProfessionalRegistration.getcareplanSMBG(email, date).then(function(response) {
	...
	db.user.authUser(token).then(function(response) {
			if (!!token) {
				if (response != '' && response != null) {
                  ...
                  *****************/
                   email_web_notification(professionalID ,userID, SMBGVal, SMBPVal);
                  /****************
                  ...
                  *****************/
					...
					/////Add revies
					var sqlinsert = "INSERT INTO carePlanreview (`professionalID`, `userID`, `comment`, `SMBGVal`, `SMBPVal`, `review_report`) VALUES ('" + professionalID + "' ,'" + userID + "', '" + comment + "', '" + SMBGVal + "', '" + SMBPVal + "', '"+review_report+"')";
...
function email_web_notification(professionalID, userID, SMBGVal, SMBPVal){

var email='';

if(SMBGVal=='make an appointment to see your doctor For SMBG.'){

///add notification to web
		var sql = "INSERT INTO notification (`fromUserID`, `toUserID`, `subject`, `details`) values ";
		sql += "('" + professionalID + "','" + userID + "','" + SMBGVal + "','" + SMBGVal + "')";
		db.notification.addnotification(sql).then(function(response) {
...
if(SMBPVal=='make an appointment to see your doctor For SMBP.')
{

	///add notification to web
		var sql = "INSERT INTO notification (`fromUserID`, `toUserID`, `subject`, `details`) values ";
		sql += "('" + professionalID + "','" + userID + "','" + SMBPVal + "','" + SMBPVal + "')";
		db.notification.addnotification(sql).then(function(response) {
	...
	db.user.checkByuserid(userID).then(function(response){
		...
		email=response[0].email;
			...
			*************************/
			...
			html: SMBGVal + " " + SMBPVal + " <br><br>Best Regards <br>CHIEF Support<br><br><hr><br>This message was sent to " + email + ". If you don't want to receive these emails from CHIEF in the future, please update your notification settings." // body
			}, function(error, response) { //callback
			if (error) {
			console.log(error);
			} else {
