	...
	db.user.authUser(token).then(function(response) {
			if (!!token) {
				if (response != '' && response != null) {
					...
					sqlDelete = "DELETE FROM bodydensity where recordDateTime IN  (" + array_dateTime + ")";
					/********** End Delete *********************************/

					db.bodydensity.delete_body_density(sqlDelete).then(function(response) {
			...
			if (!!token) {
				if (response != '' && response != null) {
					...
					for (var i = 0; i < total - 1; i++) {
						...
						db.bodydensity.updatebodydensity(sql).then(function(response) {}).error(function(err) {
							res.json(err);
						});
						sql = sql.substr(0, sql.length);
					}
			...
			if (!!token) {
				if (response != '' && response != null) {
					...
					for (var i = 0; i < total - 1; i++) {
						...
						db.bodydensity.deletebodydensity(sql).then(function(response) {}).error(function(err) {
							res.json(err);
						});
